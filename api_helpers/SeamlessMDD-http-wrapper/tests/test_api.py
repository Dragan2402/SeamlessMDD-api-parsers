...
def test_check_exists_found(client: FlaskClient, sample_file_path: str):
    resp = client.get(
        "/check-exists", query_string={"id": "1", "file_path": sample_file_path}
    )
    assert resp.status_code == 200
    data = resp.get_json()
    assert data and data.get("exists") is True
    assert data.get("element")


def test_check_exists_not_found(client: FlaskClient, sample_file_path: str):
    resp = client.get(
        "/check-exists", query_string={"id": "999", "file_path": sample_file_path}
    )
    assert resp.status_code == 200
    data = resp.get_json()
    assert data and data.get("exists") is False
    assert data.get("element") is None
...
def test_get_by_path_not_implemented(client: FlaskClient, sample_file_path: str):
    resp = client.get(
        "/get-by-path",
...
def test_replace_by_id_success(client: FlaskClient, sample_file_path: str):
    payload = {"id": "1", "new_element_html": "<div>New Content</div>"}
    resp = client.post(
        "/replace-by-id", json=payload, query_string={"file_path": sample_file_path}
    )
    assert resp.status_code == 200
    assert resp.get_json().get("message") == "Element replaced successfully"


def test_remove_by_id_success(client: FlaskClient, sample_file_path: str):
    resp = client.delete(
        "/remove-by-id", query_string={"id": "1", "file_path": sample_file_path}
    )
    assert resp.status_code == 200
    assert resp.get_json().get("message") == "Element removed successfully"


def test_get_by_jinja_variable_always_200(client: FlaskClient, sample_file_path: str):
    resp = client.get(
        "/get-by-jinja-variable",
        query_string={"variable_name": "foo", "file_path": sample_file_path},
    )
    assert resp.status_code == 200
    data = resp.get_json()
    assert data and "elements" in data and isinstance(data["elements"], list)
...
def test_update_element_by_path_success(client: FlaskClient, sample_file_path: str):
    payload = {
        "old_element_path": "/html/body/div[1]",
        "new_element_path": "/div",
        "new_element_content": "<div class='container'>Updated Content</div>",
        "important_data": {"text": True},
    }
    resp = client.post(
    ...
    assert resp.status_code == 200
    assert resp.get_json().get("message") == "Element updated successfully"
...
def test_insert_element_by_path_success(client: FlaskClient, sample_file_path: str):
    # Insert a new list item under the existing <ul>
    payload = {
...
def test_check_if_node_exists_true(client: FlaskClient, sample_file_path: str):
    payload = {
        "xpath": "/html/body/div[1]/ul/li",
        "node": "<li>Field (F1)</li>",
    }
    resp = client.post(
